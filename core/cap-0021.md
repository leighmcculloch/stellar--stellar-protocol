...
sign and exchange a closing transaction C_{p,i}, and finally 3) sign and
exchange a declaration transaction D_{p,i}.  The transactions are
constructed as follows:

* D_{p,i}, the _declaration transaction_, declares an intent to execute
  the corresponding closing transaction C_{p,i}.  D_{p,i} has source account
  E, sequence number 2i, `minSeqNum` set to s, with all operations sponsored by p.
  Hence, D_{p,i} can
  execute at any time, so long as E's sequence number n satisfies s <=
  n < 2i.  D_{p,i} always leaves E's sequence number at 2i after
  executing.  Because C_{p,i} has source account E and sequence number
  2i+1, D_{p,i} leaves E in a state where C_{p,i} can execute.  Note that D_{p,i}
...
* C_{p,i}, the _closing transaction_, disburses funds to R and changes the
  signing weights on E such that I unilaterally controls E.  C_{p,i} has
  source account E, sequence number 2i+1, a `minSeqAge` of S (the
  synchrony period), with all operations sponsored by p.
  The `minSeqAge` prevents a misbehaving party
  from executing C_{p,i} when the channel state has already progressed to
  a later iteration number, as the other party can always invalidate
  C_{p,i} by submitting D_{p,i}' for some i' > i.  C_{p,i} contains one or more
...
They then exchange C_{p,i} and D_{p,i} (which unlike the update case, can be
exchanged in a single phase of communication because D_{p,i} is not yet
...
To close the channel cooperatively, the parties re-sign C_{p,i} with a
