...
`organization.beneficial_owners.{id}.person.{SEP-0009 person field}` | vary | Organization beneficial owner fields for persons
`organization.beneficial_owners.{id}.person.control_person` | boolean | Indicates if a beneficial owner is also a control person
`organization.beneficial_owners.{id}.person.control_person_title` | string | If a beneficial owner is control person then this field will store the control person title
`organization.beneficial_owners.{id}.person.ownership_percentage` | string | Holds the percentage owned by the beneficial owner. Must be a number in the range 0 - 100 with at most 2 decimal places
`organization.beneficial_owners.{id}.organization.{SEP-0009 organization field}`| vary | Organization beneficial owner fields for organizations
`organization.beneficial_owners.{id}.organization.ownership_percentage` | string | Holds the percentage owned by the beneficial owner. Must be a number in the range 0 - 100 with at most 2 decimal places
...
Beneficial owners are individuals or entities that have ownership or control. They may be:

* Persons - Individuals which hold ownership.
* Organizations - Entites which hold ownership.
* Control Person - Control persons who are able to use their authority or influence to make decisions on the corporation's activities. Control persons include but are not limited to senior managers, members of the board of directors, and officers such as the CEO and CFO. A control person is also called an affiliated person.
