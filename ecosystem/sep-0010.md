...
- The **Client Application**: the application used by the owner of the Stellar account being authenticated.
...
1. The **Client Application** verifies that the transaction's first operation is a Manage Data operation that has its:
   1. Source account set to the user's Stellar account.
...
1. The **Client Application** signs the transaction using the secret key(s) of signers for the user's Stellar account
1. The **Client Application** submits the signed challenge back to the **Web Auth Server** using [`token`](#token) endpoint
1. The **Web Auth Server** checks that the user's Stellar account exists
1. If the user's Stellar account exists:
  1. The **Web Auth Server** gets the signers of the user's account
  1. The **Web Auth Server** verifies that one or more signatures are from signers of the user's Stellar account.
  ...
  1. The **Web Auth Server** verifies the weight provided by the signers of the user's Stellar account meets the required threshold(s), if any
1. If the user's account does not exist (optional):
  1. The **Web Auth Server** verifies the signature count is two
  1. The **Web Auth Server** verifies that one signature is correct for the master key of the user Stellar account
...
1. If the signatures check out, the **Web Auth Server** responds with a [JWT](https://jwt.io) that represents the user's session
...
* The **Web Auth Server** can verify that the **Client Application** holds the secret key(s) to signers of the user's Stellar account
* The **Client Application** is able to prove their identity using a Ledger or other hardware wallet as well as by having direct access to the secret key(s)
* The **Web Auth Server** can choose its own timeout for the user's session
* The **Web Auth Server**can choose required threshold(s) that the user needs on the account, if any
...
The organization running the **TOML Server** indicates that it supports user authentication via this protocol by specifying `WEB_AUTH_ENDPOINT` in their [`stellar.toml`](sep-0001.md) file. This is how a wallet knows where to find the **Web Auth Server**. A **Web Auth Server** is required to implement the following behavior for the web authentication endpoint:
...
* `transaction`: an XDR-encoded Stellar transaction with the following:
  ...
  * operations:
    * `manage_data(source: user_account, key: '<home domain> auth', value: random_nonce())`
...
* if the operation's source account does not exist:
  ...
  * verify that remaining signature is correct for the master key of the user's Stellar account;
...
An account's signers may include the **TOML Server**'s ``SIGNING_KEY`` if the server is a signer for the account. When determining the weight of the remaining sigantures the signature from ``SIGNING_KEY`` should be explicitly excluded. A **Web Auth Server** should not assist a user in authentication.
