...
- A **Home Domain**: a domain hosting a [SEP-1 stellar.toml](sep-0001.md) containing a `WEB_AUTH_ENDPOINT` (URL) and `SIGNING_KEY` (`G...`). The ``WEB_AUTH_ENDPOINT`` must be hosted by a **Web Auth Endpoint** as described below.
- A **Web Auth Endpoint**: a domain hosting an endpoint providing the GET and POST operations discussed in this document. The endpoint may be hosted on a **Home Domain**'s, a sub-domain of a **Home Domain**, or any other domain.
...
To validate the challenge transaction the following steps are performed by the **Web Auth Endpoint**. If any of the listed steps fail, then the authentication request must be rejected — that is, treated by the **Web Auth server** as an invalid input.
