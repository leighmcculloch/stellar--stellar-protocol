...
* The server can verify that the client holds the secret key(s) of the user's Stellar account
...
A server may optionally choose to only issue JWTs to a client whose signatures meet a threshold.
...
Servers may add claims to capture the state of the account, threshold of control, signers that the user signed the challenge transaction, or any other claims that are specific to the application, see [RFC7519].
...
Services that use the JWT as proof the user meets a threshold of control of a Stellar account may require additional claims be included so that they can check that the signers of the challenge transaction and their weights continue to meet any requirements the service has. The service would need to check the state of the account on the Stellar network.
