...
- A **Web Auth Domain**: the domain where the **Web Auth Endpoint** is hosted.
- A **Web Auth Server**: the server providing the **Web Auth Endpoint** and its functionality.
- A **Server Account**: the Stellar account used by the **Web Auth Server** during mutual authentication, and specified in the **Home Domain**'s [SEP-1 stellar.toml](sep-0001.md) as the `SIGNING_KEY`.
- An **Authenticating Account**: a Stellar account being authenticated.
- A **Client Application**: the software used by the account holder of the **Authenticating Account** to authenticate with the **Web Auth Server**.
...
1. The **Client Application** looks up the `WEB_AUTH_ENDPOINT` and `SIGNING_KEY` from the `stellar.toml`. The `SIGNING_KEY` discovered is the **Server Account**.
...
1. The **Client Application** verifies that the transaction is signed by the **Server Account**.
...
1. The **Client Application** verifies that if the transaction has a Manage Data operation with key `web_auth_domain` that it has:
   ...
   1. Value set to the **Web Auth Domain**.
1. The **Client Application** verifies that if the transaction has other operations they are Manage Data operations that all have their source accounts set to the **Server Account**.
...
1. If the **Authenticating Account** exists:
  ...
  1. The **Web Auth Server** verifies that there is only one additional signature from the **Server Account**.
...
1. If the **Authenticating Account** does not exist (optional):
  ...
  1. The **Web Auth Server** verified that the other signature is **Server Account**.
...
* The **Client Application** can verify that the **Web Auth Server** holds the secret key to the **Server Account**.
...
`home_domain` | string | (optional) a **Home Domain**. Servers that generate tokens for multiple **Home Domain**s can use this parameter to identify which home domain the **Client Application** hopes to authenticate with. If not provided by the **Client Application**, the **Web Auth Server** should assume a default for backwards compatibility with older **Client Applications**.
...
* `transaction`: an XDR-encoded Stellar transaction with the following:
  ...
  * operations:
    * `manage_data(source: authenticating_account, key: '<home domain> auth', value: random_nonce())`
      * The source account is the **Authenticating Account**.
    ...
    * subsequent operations, order unimportant:
      * `manage_data(source: server_account, key: 'web_auth_domain', value: web_auth_domain)`
        ...
        * The value is the **Web Auth Domain**. It can be at most 64 characters.
      * zero or more `manage_data(source: server_account, ...)` reserved for future use
  * signature by the **Server Account**
* `network_passphrase`: (optional but recommended) Stellar network passphrase used by the **Home Domain**. This allows a **Client Application** to verify that it's using the correct passphrase when signing and is useful for identifying when a **Client Application** or **Web Auth Server** have been configured incorrectly.
...
An account's signers may include the **Server Account** if the server is a signer for the account. When determining the weight of the remaining sigantures the signature from **Server Account** should be explicitly excluded. A **Web Auth Server** should not assist in authentication.
...
A **Web Auth Server** that needs to verify that the **Client Application** has authority aligned with the capability to move money out of an **Authenticating Account** can verify that the medium threshold is met. It should do this by checking that the sum of the weights of the challenge transaction signers is equal or greater to the medium threshold.
...
A **Web Auth Server** that needs to verify the **Client Application** has complete authority of an **Authenticating Account** should verify that the weight of the client signatures meet the high threshold. It should do this by checking that the sum of the weights is equal or greater to the high threshold.
