...
1. The client signs the transaction using the secret key(s) of the user's Stellar account
...
* The server can verify that the client holds the secret key(s) to their account
...
* The client is able to prove their identity using secret key(s) that are signers and not the master key of the account
