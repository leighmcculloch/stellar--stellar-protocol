...
- The **TOML Server**: a server hosting a [SEP-1 stellar.toml](sep-0001.md) containing a `WEB_AUTH_ENDPOINT` (URL) and `SIGNING_KEY` (`G...`). The ``WEB_AUTH_ENDPOINT`` must be hosted by the **Web Auth Server** as described below.
- The **Web Auth Server**: a domain hosting an endpoint providing the GET and POST operations discussed in this document. The endpoint may be hosted on the **TOML Server**'s domain, a sub-domain of the **TOML Server**'s domain, or any other domain.
- The **Authenticating Account**: The public key of the Stellar account being authenticated.
- The **Client Application**: the application used by the owner of the **Authenticating Account**.

The discovery flow is as follows:

1. The **Client Application** retrieves the `stellar.toml` from the **TOML Server** in accordance with [SEP-1 stellar.toml](sep-0001.md).
1. The **Client Application** looks up the `WEB_AUTH_ENDPOINT` and `SIGNING_KEY` from the `stellar.toml`.

The authentication flow is as follows:

1. The **Client Application** obtains a unique [`challenge`](#challenge) from the **Web Auth Server**, which is represented as specially formed Stellar transaction
...
1. The **Client Application** verifies that the transaction's first operation is a Manage Data operation that has its:
   ...
   1. Key set to `<home domain> auth` where the home domain is the **TOML Server**'s domain.
...
1. The **Client Application** verifies that if the transaction has a Manage Data operation with key `web_auth_domain` that it has:
   1. Source account set to the ``SIGNING_KEY`` from the **TOML Server**'s [SEP-1 stellar.toml](sep-0001.md).
   1. Value set to the **Web Auth Server**'s domain.
1. The **Client Application** verifies that if the transaction has other operations they are Manage Data operations that all have their source accounts set to the the ``SIGNING_KEY`` from the **TOML Server**'s [SEP-1 stellar.toml](sep-0001.md).
1. The **Client Application** signs the transaction using the secret key(s) of the signer(s) for the **Authenticating Account**
1. The **Client Application** submits the signed challenge back to the **Web Auth Server** using [`token`](#token) endpoint
1. The **Web Auth Server** checks that the **Authenticating Account** exists
1. If the **Authenticating Account** exists:
  1. The **Web Auth Server** gets the signers of the **Authenticating Account**
  1. The **Web Auth Server** verifies that one or more signatures are from signers of the **Authenticating Account**.
  1. The **Web Auth Server** verifies that there is only one additional signature from the ``SIGNING_KEY`` on the **TOML Server**'s [SEP-1 stellar.toml](sep-0001.md).
  1. The **Web Auth Server** verifies the weight provided by the signers of the **Authenticating Account** meets the required threshold(s), if any
1. If the **Authenticating Account** does not exist (optional):
  1. The **Web Auth Server** verifies the signature count is two
  1. The **Web Auth Server** verifies that one signature is correct for the master key of the **Authenticating Account**
  1. The **Web Auth Server** verified that the other signature is from the ``SIGNING_KEY`` on the **TOML Server**'s [SEP-1 stellar.toml](sep-0001.md).
1. If the signatures check out, the **Web Auth Server** responds with a [JWT](https://jwt.io) that represents the authenticated session

The flow achieves several things:

* Both **Client Application** and **Web Auth Server** can be implemented using well-established Stellar libraries
* The **Client Application** can verify that the **Web Auth Server** holds the secret key to the ``SIGNING_KEY`` from the **TOML Server**'s [SEP-1 stellar.toml](sep-0001.md).
* The **Web Auth Server** can verify that the **Client Application** holds the secret key(s) to signer(s) of the **Authenticating Account**
* The **Web Auth Server** can choose its own timeout for the authenticated session
* The **Web Auth Server** can choose required signing threshold(s) that must be met, if any
* The **Web Auth Server** can choose to include other application-specific claims
* The **Web Auth Server** can choose to authenticate Stellar accounts that do not yet exist

## Authentication Endpoint

The organization running the **TOML Server** indicates that it supports authentication via this protocol by specifying `WEB_AUTH_ENDPOINT` in their [`stellar.toml`](sep-0001.md) file. This is how a wallet knows where to find the **Web Auth Server**. A **Web Auth Server** is required to implement the following behavior for the web authentication endpoint:
...
This endpoint must respond with a Stellar transaction signed by the ``SIGNING_KEY`` from the **TOML Server**'s [`stellar.toml`](sep-0001.md) file that has an invalid sequence number (0) and thus cannot be executed on the Stellar network. The **Client Application** can then sign the transaction using standard Stellar libraries and submit it to [`token`](#token) endpoint to prove that it controls the **Authenticating Account**. This approach is compatible with hardware wallets such as Ledger. The **Client Application** must also verify the home domain in the first operation of the challenge is the domain of the **TOML Server**.
...
----------|---------------|------------
`account` | `G...` string | The stellar account that the wallet wishes to authenticate with the **Web Auth Server** 
`home_domain` | string | (optional) The home domain of the **TOML Server**. Servers that generate tokens for multiple **TOML Servers** can use this parameter to identify which home domain the **Client Application** hopes to authenticate with. If not provided by the **Client Application**, the **Web Auth Server** should assume a default for backwards compatibility with older **Client Applications**.
...
* `transaction`: an XDR-encoded Stellar transaction with the following:
  * source account set to the **TOML Server**'s ``SIGNING_KEY``
  ...
  * operations:
    * `manage_data(source: authenticating_account, key: '<home domain> auth', value: random_nonce())`
      * The value of key is the home domain of the **TOML Server**, followed by `auth`. It can be at most 64 characters.
    ...
    * subsequent operations, order unimportant:
      * `manage_data(source: server_account, key: 'web_auth_domain', value: web_auth_domain)`
        * The source account is the **TOML Server**'s ``SIGNING_KEY``, the value is the domain of the **Web Auth Server**. It can be at most 64 characters.
      * zero or more `manage_data(source: server_account, ...)` reserved for future use
  * signature by the **TOML Server**'s stellar.toml `SIGNING_KEY`
* `network_passphrase`: (optional but recommended) Stellar network passphrase used by the **TOML Server**. This allows the **Client Application** to verify that it's using the correct passphrase when signing and is useful for identifying when a **Client Application** or **Web Auth Server** have been configured incorrectly.
...
To validate the challenge transaction the following steps are performed by the **Web Auth Server**. If any of the listed steps fail, then the authentication request must be rejected — that is, treated by the **Web Auth server** as an invalid input.

* decode the received input as a base64-urlencoded XDR representation of Stellar transaction envelope;
* verify that transaction source account is equal to the **TOML Server**'s ``SIGNING_KEY``;
...
* verify that transaction's first operation is a [Manage Data](https://www.stellar.org/developers/guides/concepts/list-of-operations.html#manage-data) operation that:
  ...
  * has the **TOML Server**'s domain as the key of the operation
* verify that transaction envelope has a correct signature by the **TOML Server**'s ``SIGNING_KEY``;
...
* verify that transaction containing additional Manage Data operations have their source account set to the **TOML Server**'s ``SIGNING_KEY``
...
Upon successful verification, **Web Auth Server** responds with a session JWT, containing the following claims:
...
If the **Web Auth Server** successfully validates the submitted challenge transaction, the endpoint should return `200 OK` HTTP status code and a JSON object with the following fields:
...
An account's signers may include third-party services providing services to the owner of the **Authenticating Account**. Authenticating accounts with less than any threshold may allow a third-party to authenticate.

An account's signers may include the **TOML Server**'s ``SIGNING_KEY`` if the server is a signer for the account. When determining the weight of the remaining sigantures the signature from ``SIGNING_KEY`` should be explicitly excluded. A **Web Auth Server** should not assist in authentication.

The **Web Auth Server** should only issue a JWT if the appropriate thresholds are met, but if a **Web Auth Server** is supporting a variety of applications it may choose to use additional application specific claims to capture the threshold of control the **Client Application** has proven.

### Verifying Authority to Move Funds

A **Web Auth Server** that needs to verify that a **Client Application** has authority aligned with the capability to move money out of an **Authenticating Account** can verify that the medium threshold is met. It should do this by checking that the sum of the weights of the challenge transaction signers is equal or greater to the medium threshold.
...
A **Web Auth Server** that needs to verify a **Client Application** has complete authority of an **Authenticating Account** should verify that the weight of the client signatures meet the high threshold. It should do this by checking that the sum of the weights is equal or greater to the high threshold.

### Verifying Being a Signer

A **Web Auth Server** may choose to issue JWTs for less than all thresholds and based on any other application specific logic. It's important to keep in mind that a Stellar account may have third-parties who are signers. Authenticating accounts with less than any threshold may allow a third-party to authenticate.

### Verifying Accounts that Do Not Exist

A **Web Auth Server** that needs to support validating accounts that do not exist can require a signature of the master key of the account address for accounts that do not exist.

## JWT Expiration

Servers should select an expiration time for the JWT that is appropriate for the assumptions and risk of the interactions a **Client Application** can perform with it. A **Client Application** may be in control of an account at the time the JWT is issued but they may lose control of the account through a change in signers. Expiration times that are too long increase the risk that control on the account has changed. Expiration times that are too short increase the number of times authentication must reoccur, and a user using a hardware signing device or who must complete a complex signing process could have a poor user experience.
