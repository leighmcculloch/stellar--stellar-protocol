...
- A **Client Application**: the software used by the account holder of the **Authenticating Account**.
  ...
  * operations:
    ...
    * subsequent operations, order unimportant:
      * `manage_data(source: server_account, key: 'web_auth_domain', value: web_auth_domain)`
        * The source account is the **Home Domain**'s ``SIGNING_KEY``, the value is the **Web Auth Domain**. It can be at most 64 characters.
